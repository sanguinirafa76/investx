...
const YF_MAP = {
  ...
  'MXRF11': 'MXRF11.SA', 'HGLG11': 'HGLG11.SA', 'XPLG11':'XPLG11.SA',
  'VISC11': 'VISC11.SA',
...
async function fetchMarketData(symbols, interval = '5m', range = '1d') {
  try {
    ...
    console.warn('[market] fetchMarketData falhou:', err.message);
    return {};
  }
}
...
async function refreshRealPrices() {
  ...
  for (const cat of ['acoes', 'fiis', 'cripto', 'indices']) {
    for (const row of currentMarketData[cat]) {
      const yfSym = YF_MAP[row.tick];
      if (!yfSym) continue;
      const d = data[yfSym];
      if (!d || d.error || !d.price) continue;
...
async function updateLandingPrices() {
  const map = {
    ...
    'VALE3': { elPrice: 'lpc-vale3',  elChg: 'lpc-vale3-chg'  },
    'BTC':   { elPrice: 'lpc-btc',    elChg: 'lpc-btc-chg'    },
    'IBOV':  { elPrice: 'lpc-ibov',   elChg: 'lpc-ibov-chg'   },
    'ETH':   { elPrice: 'lpc-eth',    elChg: 'lpc-eth-chg'    },
    'ITUB4': { elPrice: 'lpc-itub4',  elChg: 'lpc-itub4-chg'  },
    'WEGE3': { elPrice: 'lpc-wege3',  elChg: 'lpc-wege3-chg'  },
    'SOL':   { elPrice: 'lpc-sol',    elChg: 'lpc-sol-chg'    },
  };
